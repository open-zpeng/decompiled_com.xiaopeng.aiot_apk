...
public class IntervalControl {
    private static final int MIN_OPERATION_INTERVAL = 500;
    ...
    public boolean isFrequently() {
        return isFrequently(500);
    }

    public boolean isFrequently(int i) {
        if (Math.abs(System.currentTimeMillis() - this.mLastPairingTime) < i) {
            com.xiaopeng.lib.utils.LogUtils.d(TAG, String.format("%s-- two operation is too short", this.mUser));
            return true;
        }
